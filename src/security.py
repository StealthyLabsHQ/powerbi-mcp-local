...
WRITE_TOOLS = {
    ...
    "pbi_repair_report_fields",
    "pbi_persist_now",
    # Reopen probe captures the entire primary screen, may close existing
    # PBI Desktop windows, and can write a PNG to an attacker-controlled
    # path. Treat as write so --readonly / deny-write actually blocks it.
...
class SecurityPolicy:
    ...
    def from_mapping(cls, mapping: dict[str, Any] | None) -> SecurityPolicy:
        ...
        return cls(
            ...
            # ``enabled_tools: []`` must mean "no tools allowed" (fail-closed).
            # Only ``null`` / missing key disables the allowlist check.
            enabled_tools={str(item) for item in enabled} if enabled is not None else None,
