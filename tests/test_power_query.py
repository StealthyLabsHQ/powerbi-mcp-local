...
class PowerQueryToolTests(unittest.TestCase):
    ...
    def test_bulk_import_excel_auto_map_skips_hidden_and_multi_partition(self) -> None:
        ...
        hidden = FakeTable(
            "HiddenTable", [FakePartition("HiddenTable", FakeQueryPartitionSource("select 1"))], is_hidden=True
        )
        sharded = FakeTable(
            "Sharded",
            [
        ...
        with (
            patch("tools.power_query.inspect_excel_archive", return_value=Path("dummy.xlsx")),
            patch(
    ...
    def test_list_power_queries_reports_partition_metadata(self) -> None:
        sales = FakeTable(
            "Sales", [FakePartition("Sales", FakeMPartitionSource("let\nin\n    Source"), source_type="M")]
        )
